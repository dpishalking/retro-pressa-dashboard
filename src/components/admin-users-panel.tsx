...
function buildAccessMessage(input: {
  ...
  return [
    ...
    input.accessLevel === "mop"
      ? "После входа откроется кабинет со шпаргалками, обучением и мотивацией."
    ...
    <main className="mx-auto w-[min(1100px,calc(100%-32px))] py-8">
      <header className="mb-8">
        ...
        <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
          {isFullAdmin
            ? "Создавайте пользователей и назначайте уровни доступа: администратор видит всё, РОП — аналитику и команду, менеджер — обучение и мотивацию. Заявки с /registration нужно одобрить, прежде чем менеджер сможет войти."
            ...
            {isFullAdmin ? (
              <label className="block text-sm font-semibold text-slate-700">
                ...
                >
                  ...
                  <option value="mop">Менеджер — обучение, мотивация, база знаний</option>
                </select>
              </label>
            ) : (
              <p className="rounded-xl border border-violet-200 bg-violet-50 px-4 py-3 text-sm text-violet-900">
                Уровень доступа: <span className="font-bold">Менеджер — обучение, мотивация, база знаний</span>
              </p>
            )}
